-- Виправлення функції process_bonus_operations_manual з корекцією форматів
...
BEGIN
    ...
    -- Оновлюємо баланс клієнта (конвертуємо назад в копійки)
    IF spent_bonus > 0 OR earned_bonus > 0 THEN
        UPDATE clients
        SET bonus = ROUND(client_current_bonus_grn * 100, 0),  -- Зберігаємо в копійках
            updated_at = NOW()
        WHERE client_id = p_client_id;
    END IF;
END;
        ...
        IF TG_OP = 'INSERT' THEN
            ...
            IF spent_bonus > 0 THEN
                INSERT INTO transaction_bonus (
                    ...
                    NEW.client_id, NEW.transaction_id, 'SPEND', -spent_bonus,
                    client_current_bonus_grn, client_current_bonus_grn - spent_bonus,
            ...
            IF earned_bonus > 0 THEN
                INSERT INTO transaction_bonus (
                    ...
                    NEW.client_id, NEW.transaction_id, 'EARN', earned_bonus,
                    client_current_bonus_grn, client_current_bonus_grn + earned_bonus,
            ...
            IF spent_bonus > 0 OR earned_bonus > 0 THEN
                ...
                -- Логуємо операцію
                RAISE NOTICE 'Bonus operation for client % (transaction %): earned=%, spent=%, new_balance_grn=%',
                    NEW.client_id, NEW.transaction_id, earned_bonus, spent_bonus, client_current_bonus_grn;
            END IF;
