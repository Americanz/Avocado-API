...
from src.core.models import User, Log, APILog
from src.core.models import LogType
from src.config.settings import Settings
from ..common.ctx import CTX_USER_ID
from ..common.bgtask import BgTasks
...
class APILoggerAddResponseMiddleware(SimpleBaseMiddleware):
    """
    Need to be used with APILoggerMiddleware
    """
    ...
    async def after_request(self, request: Request, response: dict) -> None:
        if (
            ...
            try:
                ...
                logger.warning(
                    f"Failed to decode response body for API log {request.state.api_log_id}"
                )
            except Exception as e:
                logger.error(
                    f"Error processing API log {request.state.api_log_id}: {str(e)}"
                )
